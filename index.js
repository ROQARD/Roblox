export default {
  async fetch(request) {
    ...
    if (url.pathname === "/api/get-stats") {
      ...
      // We use RoProxy domains here because Roblox blocks Cloudflare IPs directly.
      ...
      try {
        ...
        if (!uniRes.ok) throw new Error("ID_NOT_FOUND");
        const uniData = await uniRes.json();
        const uId = uniData.universeId;

        // Step 2: Multi-Fetch (parallel)
        const [gameRes, voteRes, favRes, iconRes] = await Promise.all([
          ...
          fetch(`https://thumbnails.${PROXY}/universes/v1/icons?universeIds=${uId}&size=150x150&format=Png&isCircular=false`)
        ]);
        ...
        if (!d) throw new Error("NO_GAME_DATA");
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        #status { margin-top: 15px; font-size: 0.8rem; font-weight: 600; min-height: 20px; }
        .recent-container { margin-top: 20px; text-align: left; display: none; }
        .recent-title { font-size: 0.7rem; color: var(--text-dim); text-transform: uppercase; margin-bottom: 10px; letter-spacing: 1px; font-weight: 700; }
        .recent-list { display: flex; gap: 10px; overflow-x: auto; padding-bottom: 5px; }
        .recent-item {
            background: var(--glass);
            border: 1px solid var(--border);
            border-radius: 10px;
        ...
        .recent-item img { width: 24px; height: 24px; border-radius: 4px; }
        .recent-item span { font-size: 0.8rem; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .result-card { display: none; flex-direction: column; gap: 12px; animation: slideUp 0.4s ease-out; }
        .header-box {
            background: var(--glass);
            border: 1px solid var(--border);
            padding: 20px;
        ...
        .header-box img { width: 80px; height: 80px; border-radius: 12px; border: 1px solid var(--border); }
        ...
        .label { font-size: 0.65rem; color: var(--text-dim); text-transform: uppercase; font-weight: 800; margin-bottom: 5px; }
        .value { font-size: 1.3rem; font-weight: 800; }
        .footer-credit { position: fixed; bottom: 20px; right: 25px; font-size: 0.7rem; font-weight: 800; color: var(--text-dim); opacity: 0.5; }
    ...
    <div id="results" class="result-card">
        <div class="header-box">
            ...
            <div style="text-align:left;">
                <div id="gName" style="font-weight:800; font-size:1.3rem;">Experience</div>
                <div id="gCreator" style="color:var(--accent); font-size:0.85rem; font-weight:600;">By Creator</div>
            </div>
        </div>
        <div class="grid-stats">
            ...
            <div class="stat-item full-width">
                ...
                <div id="gDesc" style="font-size: 0.8rem; color: var(--text-dim); line-height: 1.4; max-height: 80px; overflow-y: auto;">-</div>
            </div>
        </div>
    </div>
</div>
...
<script>
    const RECENT_KEY = "rostats_history_v2";
    ...
    async function fetchStats() {
        ...
        if (!id) {
            status.innerText = "Please enter a valid ID.";
        ...
        try {
            ...
            document.getElementById('gDesc').innerText = data.game.description || "No description provided.";
